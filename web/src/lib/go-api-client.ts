...
class GoApiClient {
  ...
  /**
   * Update mentor's own profile visibility status (using session auth)
   */
  async mentorUpdateProfileStatus(
    cookies: string,
    payload: UpdateProfileStatusRequest
  ): Promise<UpdateProfileStatusResponse> {
    const { data } = await this.requestWithCookies<UpdateProfileStatusResponse>(
      'POST',
      '/api/v1/mentor/profile/status',
      { cookies, body: payload as unknown as Record<string, unknown> }
    )
    return data
  }
