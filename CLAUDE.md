...
- `infra/` — Compose/Traefik deployment + observability. Verify with `make check` from `infra/` (compose renders + per-service env allowlist + backup sidecar tests; falls back to `.env.example` when there is no `.env`).
