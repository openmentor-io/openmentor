...
#   --staging    deploy to the staging VM (VM_SSH_*_STAGING + DOMAIN_STAGING)
#   -h, --help   show help
#
...
usage() {
    ...
    echo "  --staging          Deploy to the staging VM (VM_SSH_*_STAGING + DOMAIN_STAGING)"
...
# Validate required variables. The domain is required because step 9/9's probe
# is the only check that traverses Traefik; refuse now, not after the push.
REQUIRED_VARS=(
    "ECR_REGISTRY"
    "AWS_REGION"
    "VM_SSH_HOST"
...
# Everything downstream uses the _-prefixed target, never the raw production
# variable — otherwise a --staging run probes and reports production's host.
_VM_SSH_HOST="$VM_SSH_HOST"
_VM_SSH_USER="$VM_SSH_USER"
_VM_SSH_KEY_FILE="${VM_SSH_KEY_FILE:-}"
_DOMAIN="$DOMAIN"
_DEPLOYMENT_NAME=production

if [ "$STAGING" = true ]; then
    _VM_SSH_HOST="$VM_SSH_HOST_STAGING"
    _VM_SSH_USER="$VM_SSH_USER_STAGING"
    _VM_SSH_KEY_FILE="${VM_SSH_KEY_FILE_STAGING:-}"
    _DOMAIN="${DOMAIN_STAGING:-}"
    _DEPLOYMENT_NAME=staging
    # The "deploying to STAGING" warning is printed below, after the required-var
    # check, so it can name the domain it resolved to as well as the SSH target.
fi
...
if [ "$STAGING" = true ]; then
    echo -e "${YELLOW}⚠️  Deploying to STAGING at $_VM_SSH_HOST (https://$_DOMAIN)${NC}"
    echo -e "${YELLOW}   Only the SSH target and DOMAIN are staging-specific: every other${NC}"
    echo -e "${YELLOW}   runtime value still comes from .env.production.${NC}"
fi
...
# Derived from the deploy target, appended last so it wins over the template:
# --staging uploads this same .env.production to a different VM, so this is the
# only thing that keeps the two apart in the shared Grafana Cloud tenant (the
# per-deployment backup-freshness alert depends on it).
echo "DEPLOYMENT_NAME=$_DEPLOYMENT_NAME" >> "$TEMP_ENV_FILE"

if [ "$STAGING" = true ]; then
    # Compose interpolates DOMAIN into Traefik's router rule and ACME cert, so
    # a staging VM must be told its own hostname or it serves production's
    # (and step 9/9 could never pass).
    sed '/^DOMAIN=/d' "$TEMP_ENV_FILE" > "$TEMP_ENV_FILE.staging"
    echo "DOMAIN=$_DOMAIN" >> "$TEMP_ENV_FILE.staging"
...
# The remote deploy logic lives in deploy-remote.sh — the single canonical
# script shared with .github/workflows/deploy.yml. The LOCAL checked-out
# copy is piped over ssh stdin (never the rsynced copy on the VM, which
# app-only deploys don't refresh). Compose convergence recreates ONLY the
...
#
# Mandatory and fatal: deploy-remote.sh only probes container loopback, so a
# broken cert, router or middleware still looks healthy to it.
# --------------------------------------------------------------------------
echo -e "${BLUE}🔍 Step 9/9: Verifying public endpoint...${NC}"

URL="https://$_DOMAIN/api/healthcheck"
...
echo "  • URL: https://$_DOMAIN (verified: HTTP 200 through Traefik)"
echo ""
echo "🔗 Next steps:"
echo "  1. Monitor application at https://$_DOMAIN"
