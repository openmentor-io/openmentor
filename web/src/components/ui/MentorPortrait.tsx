 ...
 * - 'hero'  — background-removed cut-out (the <slug>/hero alpha PNG),
 *   bottom-anchored; if the cutout asset is missing it degrades to 'frame'
...
export default function MentorPortrait({
  ...
  // The hero cutout (<slug>/hero alpha PNG) is generated by the cutout
  // pipeline and may be absent (older photos, backfill not yet run). If it
  // 404s we degrade to the plain 'frame' treatment rather than initials.
  const [heroFailed, setHeroFailed] = useState(false)
  const version = updatedAtToVersion(mentor.updatedAt)
  const src = imageLoader({ src: mentor.slug, quality, version })
  const heroSrc = imageLoader({ src: mentor.slug, quality: 'hero', version })
  const useHero = mentor.photoStyle === 'hero' && !heroFailed
      ...
      {photoFailed ? (
        ...
        >
          <Image
            ...
            className="[filter:contrast(1.03)]"
