...
3. **Container registry credentials** — currently Yandex Container Registry
   ...
   - Registry ID (`crp...`)
4. **Sibling repos** cloned next to this one (`../openmentor`,
   `../openmentor-api`) with the commits you want to ship checked out —
   the image tag is each repo's short commit SHA
5. **All changes committed** in those repos (tags come from `git rev-parse`)
...
The VM must have Docker + docker-compose and this repo checked out at
`/opt/openmentor-infra`.
...
2. Build Docker images locally (frontend and/or backend), tagged with the
   source repos' short commit SHAs (`DOCKER_TAG_POLICY.md`)
...
```bash
cd ../openmentor && docker build .        # see the full frontend error
cd ../openmentor-api && docker build .    # see the full backend error
```
...
- Deploy from clean, committed trees in both app repos
