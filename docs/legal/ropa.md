...
**Sub-processor list (for privacy policy):** Hetzner (hosting), AWS SES (email), storage provider (R2/S3 — TBD P6), PostHog, Grafana Labs (observability), Cloudflare (DNS/CDN, Turnstile captcha), Calendly/Koalendar/Calendlab (only when a mentor embeds them — data goes subject→provider directly).
