...
| 5 | Product analytics | All visitors | Pseudonymous events, device data | Product improvement | Consent (cookie banner, P5.2) | PostHog (EU cloud preferred), Mixpanel (TBD — consider dropping to simplify) | Provider defaults | per provider |
...
**Sub-processor list (for privacy policy):** Hetzner (hosting), AWS SES (email), storage provider (R2/S3 — TBD P6), PostHog, Mixpanel (if kept), Google (ReCAPTCHA), Grafana Labs (observability), Cloudflare (DNS/CDN), Calendly/Koalendar/Calendlab (only when a mentor embeds them — data goes subject→provider directly).

**Open items:** D7 controller identity & governing law; D13 retention numbers; decide Mixpanel keep/drop; pick EU regions for SES/Grafana; DPAs to accept with each provider (checklist in 04-legal-compliance.md §4.2).
